...
def pil_to_bytes(image: PIL.Image.Image, format, quality=100) -> bytes:
    frame_data = io.BytesIO()
    image.save(frame_data, format=format, quality=quality)
    return frame_data.getvalue()


def pil_to_frame(image: PIL.Image.Image, format: str, quality=100) -> bytes:
